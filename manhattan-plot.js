import { s as select$1, S as Stanza, d as downloadSvgMenuItem, a as downloadPngMenuItem, b as appendCustomCss, c as defineStanzaElement } from './index-6c9e96c8.js';
...
function showLoadingIcon(element) {
  if (element.offsetHeight < 30) {
    select$1(element).transition().duration(100).style("min-height", "30px");
  }
...
function hideLoadingIcon(element) {
  style?.remove();
  select$1(element).select("#metastanza-loading-icon-div").remove();
}

function displayApiError(element, error) {
  select$1(element).select(".metastanza-error-message-div").remove();
  const p = select$1(element)
    .append("div")
